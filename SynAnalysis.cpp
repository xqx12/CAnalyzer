...
int xuyongjian[Max_Proc][Max_Length];
...
void initGrammer()
{
    ...
    memset(xuyongjian,-1,sizeof(xuyongjian));
...
//求已经分解的产生式对应的xuyongjian集,注意xuyongjian集中不能含有空($),因而Type=2
void Select()
{
    for(int i = 1; i<=procNum; i++) //j代表第几个产生式
    {
        ...
        rightLength--;
        //如果右部推出式的长度为1并且是空,xuyongjian[i-1] = follow[左边]
        if(rightLength == 1 && proc[i][rightLength + 2] == GRAMMAR_NULL)
        {
            merge(xuyongjian[i-1],follow[h],2);
        }
        //如果右部不是空的时候,xuyongjian[i-1] = first[右部全部]
        //如果右部能够推出空，xuyongjian[i-1] = first[右部全部] ^ follow[左边]
        else
        {
            int temp2[Max_Length];
            int n = 0;
            ...
            connectFirst[0] = -1;//应该重新初始化一下
            connectFirstSet(temp2);
            merge(xuyongjian[i-1],connectFirst,2);
            for(n=1; n<=rightLength; n++)
            {
                emptyRecu[0] = -1;
                result *= reduNull(proc[i][n+2]);
            }
            //如果右部能推出空，将follow[左边]并入xuyongjian[i-1]中
            if(result == 1)
            {
                merge(xuyongjian[i-1],follow[h],2);
            }
    ...
    for(int i=0; i<procNum; i++)
    {
        ...
        for(int j=0;; j++)
        {
            ...
            printf("%s ",searchMapping(xuyongjian[i][j]));
        }
        printf("\n");
    }
}
...
void MTable()
{
    ...
    for(int i=0; i<procNum; i++)
    {
        ...
        for(int j=0;; j++)
        {
            ...
            {
                if((int)(terMap[k].second) == xuyongjian[i][j])
                {
                    int n = 0;
